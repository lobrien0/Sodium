...
#
#   Program Description:
#       The following will generate a password list of common, worded passwords.

file_path = 'password_dump.txt'
...
word_list:list = [
	...
	'none','nor','north','not','note','nothing','notice','now','nt','number',
...
with open(file_path, 'w') as file:
    ...
    for state_index in range(len(states_list)):
        for number in range(10000):
            file.write(f'{states_list[state_index]}{number}\n')
            
    for x in range(len(word_list)):
        file.write(f'{word_list[x]}\n')
        print(f'On X:\t{x}')
        for y in range(len(word_list)):
            file.write(f'{word_list[x]}{word_list[y]}\n')
